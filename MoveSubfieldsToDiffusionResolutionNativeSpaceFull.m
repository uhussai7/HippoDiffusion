...
for subfield=1:5
    subfields_nii(subfield)=load_untouch_nii(sprintf('..\\Subfields\\%s\\%s\\sub%d.nii.gz',subject,LR,subfield));
end
...
Fs_xyz=scatteredInterpolant(x_m',y_m',z_m',s','nearest','nearest');

%get hippocampus mask at 
mask_nii=load_untouch_nii(sprintf('..\\Diffusion\\%s\\anat\\Native\\%s\\nodif_brain_mask.nii.gz',subject,LR));
...
inds_sub=find(mask_nii.img==1);
[x_vox y_vox z_vox]=ind2sub(size(mask_nii.img),inds_sub);


for p=1:size(inds_sub)
     temp=World(mask_nii,x_vox(p),y_vox(p),z_vox(p));
    %if(inShape(Hippo_alpha,temp(1),temp(2),temp(3)))
        testvoxx(p)=temp(1);testvoxy(p)=temp(2);testvoxz(p)=temp(3);
...
mkdir(sprintf('..\\Diffusion\\%s\\anat\\Native\\Subfields\\%s',subject,LR));

for subfield=1:6
    subb=double(sub{subfield});
    sub_nii=make_nii(subb);
    sub_nii.hdr=mask_nii.hdr;
    save_nii(sub_nii,sprintf('..\\Diffusion\\%s\\anat\\Native\\Subfields\\%s\\sub%d.nii.gz',subject,LR,subfield));
end
