...
for si=1:4
    for sj=si+1:5
        xz=load(sprintf('..\\hipposubjects_reparam_probtrackx_curvature_keep_avoid\\%s_%s_%d%d_keep_avoid_probtrack\\fdt_matrix1.dot',subject,LR,si,sj));
        coord=load(sprintf('..\\hipposubjects_reparam_probtrackx_curvature_keep_avoid\\%s_%s_%d%d_keep_avoid_probtrack\\coords_for_fdt_matrix1',subject,LR,si,sj))+1;
        M=full(spconvert(xz));
        M=M/max(M(:));
