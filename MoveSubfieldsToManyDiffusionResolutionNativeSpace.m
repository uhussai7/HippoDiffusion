...
for subfield=1:5
    subfields_nii(subfield)=load_nii(sprintf('..\\Subfields\\%s\\%s\\sub_%d-0_0_0_%s.nii.gz',subject,LR,subfield,ress(rrr)));
end
...
mask_nii=load_nii(sprintf('..\\Diffusion\\%s\\anat\\Native_%s\\Crop\\%s\\nodif_brain_mask.nii.gz',subject,resw(rrr),LR));
...
for subfield=1:5
    ...
    save_nii(sub_nii,sprintf('..\\Diffusion\\%s\\anat\\Native_%s\\Crop\\%s\\sub%d.nii.gz',subject,resw(rrr),LR,subfield));
end
